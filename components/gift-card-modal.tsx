...
export function GiftCardModal() {
  ...
  return (
    ...
    >
      <div
        className='bg-white rounded-2xl shadow-2xl p-7 w-11/12 max-w-md relative z-10 sm:w-full sm:max-w-md animate-fadeIn'
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={closeModal}
          className='absolute -top-0 right-[7px] text-blue-700 font-bold hover:text-blue-900 text-2xl'
          aria-label='Cerrar'
        >
          ✕
        </button>
        <div className='flex flex-col items-center gap-3'>
          <div className='flex justify-center mb-2'>
            <Image
              src='/images/promo-gmc.jpeg'
              alt='Promo fin de mes Autoescuela GMC'
              width={320}
              height={320}
              className='rounded-lg w-full object-contain shadow-md'
            />
          </div>
          <Link
            href='https://wa.me/2255627189?text=Hola,%20quiero%20aprovechar%20la%20promo%20de%204%20clases%20por%20%24120.000%20en%20Autoescuela%20GMC.%20%C2%BFMe%20das%20m%C3%A1s%20info?'
            target='_blank'
            className='flex items-center gap-2 bg-green-500 text-white text-center py-2 px-6 rounded-lg shadow hover:bg-green-600 transition font-bold text-sm md:text-lg mt-2'
          >
            <svg
